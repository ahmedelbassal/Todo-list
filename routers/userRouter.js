...
userRouter.post('/register', async (req, res) => {
    ...
    try {
        ...
        const { username, password, firstname, lastname, age, phone, email } = req.body
...
// get user first name by toekn returned to user from login
...
userRouter.patch('/password', async (req, res) => {
    try {
        ...
        if (isMatched) {
            ...
            const newPass = await bcrypt.hash(newPassowrd, 7);
...
// Also I will make another one that if this user is admin but I will prepare it later
...
userRouter.patch('/:id', async (req, res) => {
    ...
    try {
        ...
        // modify user data
        const userModified = await usersModel.updateOne({ _id: id }, changeDataValues).exec();
